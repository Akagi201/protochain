...
extern crate protochain_primitives;
...
native_executor_instance!(pub Executor, protochain_runtime::api::dispatch, protochain_runtime::VERSION, include_bytes!("../../runtime/wasm/target/wasm32-unknown-unknown/release/protochain_runtime.compact.wasm"));
...
mod tests {
	...
	use protochain_primitives::{Hash, BlockNumber, AccountId};
	use runtime_primitives::traits::Header as HeaderT;
	use runtime_primitives::{ApplyOutcome, ApplyError, ApplyResult, MaybeUnsigned};
	use {staking, system, consensus};
	use protochain_runtime::{Header, Block, UncheckedExtrinsic, Extrinsic, Call, Concrete, Staking,
		BuildStorage, GenesisConfig, SessionConfig, StakingConfig, BareExtrinsic};
	use ed25519::{Public, Pair};

	const BLOATY_CODE: &[u8] = include_bytes!("../../runtime/wasm/target/wasm32-unknown-unknown/release/protochain_runtime.wasm");
	const COMPACT_CODE: &[u8] = include_bytes!("../../runtime/wasm/target/wasm32-unknown-unknown/release/protochain_runtime.compact.wasm");
	...
	fn panic_execution_gives_error() {
		...
		let foreign_code = include_bytes!("../../runtime/wasm/target/wasm32-unknown-unknown/release/protochain_runtime.wasm");
	...
	fn successful_execution_gives_ok() {
		...
		let foreign_code = include_bytes!("../../runtime/wasm/target/wasm32-unknown-unknown/release/protochain_runtime.compact.wasm");
		let r = WasmExecutor::new(8).call(&mut t, &foreign_code[..], "initialise_block", &vec![].and(&from_block_number(1u64)));
		assert!(r.is_ok());
		let r = WasmExecutor::new(8).call(&mut t, &foreign_code[..], "apply_extrinsic", &vec![].and(&xt())).unwrap();
		let r = ApplyResult::decode(&mut &r[..]).unwrap();
